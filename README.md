...
2. **配置静态网络**
3. **修改SSH端口**
4. **用户安全配置**（禁用root登录、配置密码验证等）
5. **额外安全配置**（防火墙、Fail2ban等）
6. **中文语言支持**（安装中文语言包、配置中文环境）
...
- 按顺序执行各个初始化步骤：
  ...
  - 配置网络
  - 配置SSH端口
  - 配置SSH目录和authorized_keys
  - 配置SSH安全设置
  - 配置防火墙
  - 配置Fail2ban
  - 配置用户权限（创建新用户并验证）
- 记录执行日志到 `init.log` 文件
- 提示用户可以在菜单中选择禁用root账户
...
5. **配置SSH**：修改SSH端口、配置SSH目录、配置安全设置
6. **配置安全设置**：启用防火墙和Fail2ban
7. **配置用户权限**：创建并启用新账户
8. **查看执行日志**：查看详细的执行日志
9. **禁用root账户登录**：禁用root账户的登录权限（建议在确认新账户可用后执行）
10. **退出**：退出可视化界面
